...
The script first generates complete 16-bit words and then injects the corresponding 4-bit HPBS pattern into the least significant bits. For the archived `(alpha, alpha') = (0x0, 0x2)` sample in `paper/verification-0x0-0x2.txt`, 79 valid switches were found from 384 samples (20.57%). This rate is pattern-dependent and should be interpreted as an empirical filter result, not as a universal HPBS probability.
